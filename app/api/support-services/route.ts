...
export async function GET() {
  ...
  if (!result.ok) {
    return NextResponse.json({ services: [] as SupportService[] }, { status: 200 });
  }
  ...
  const products = SUPPORT_SERVICE_DEFINITIONS.flatMap(
    (definition) => {
  ...
  const services: SupportService[] = products.map((product) => ({
    id: product.id,
    title: product.name,
